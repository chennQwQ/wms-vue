<template>
  <BasicModal v-bind="$attrs" @register="registerModal" destroyOnClose :width="800" :title="getTitle" @ok="handleSubmit">
    <BasicForm @register="registerForm" name="WmsProductCategoriesForm" />
  </BasicModal>
</template>
<script lang="ts" setup>
  ...
  import { formSchema } from '../WmsProductCategories.data';
  import { loadTreeData, saveOrUpdateDict } from '../WmsProductCategories.api';
  // 获取emit
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    isSubAdd.value = !data?.isUpdate && !!data?.record?.parentId && data.record.parentId !== '0';
    if (data?.record) {
      model = unref(isUpdate) ? data.record : null;
      //表单赋值
    ...
    //父级节点树信息
    const rootTreeData = await loadTreeData({ async: false, pcode: '' });
    treeData.value = [
      {
        title: '顶级分类',
        key: '0',
        value: '0',
        parentId: '',
        children: Array.isArray(rootTreeData) ? rootTreeData : rootTreeData?.records || [],
      },
    ];
    updateSchema({
      field: 'parentId',
  ...
  function getExpandKeysByPid(pid, arr) {
    if (pid && arr && arr.length > 0) {
      for (let i = 0; i < arr.length; i++) {
        const nodeKey = arr[i].key || arr[i].value || arr[i].id;
        if (nodeKey == pid && unref(expandedRowKeys).indexOf(pid) < 0) {
          expandedRowKeys.value.push(nodeKey);
